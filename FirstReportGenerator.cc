...
void FirstReportGenerator::execute(string& outStr){
  ...
  string arr[4] = {"   ","       ","    ","      "};//to manage spacing
  ...
  for(i = 0; i < allRegions.size(); i++){
    ...
    if(r.size() == 2){ //covers all regions apart from CAN
      ...
      for(j = 0; j < allDegrees.size(); j++){
        double num = 0.0, total = 0.0;//variables to compute statistics
        for(k = 0; k < allDegrees[j]->getInfoSize() ; k++){

          //checks if reports are for All genders and for the specified region
  ...
  for(j = 0; j < allDegrees.size(); j++){
    ...
    for(k = 0; k < allDegrees[j]->getInfoSize() ; k++){
      //checks if reports are for All genders and for the specified region (CAN in this case)
      if((*allDegrees[j])[k]->getGender() == "All" &&
         (*allDegrees[j])[k]->getRegion() == r){
        num += (*allDegrees[j])[k]->getNumEmployed();
        total += (*allDegrees[j])[k]->getNumGrads();
