...
FourthReportGenerator::FourthReportGenerator(){
  reportName = "Generate employment percentage for each degree by year for all genders for a selected region in Canada";
}
    ...
    string d = allDegrees[i]->getSign();//e.g. AB
      ...
      for(j = 0; j < allYears.size(); j++){
        ...
        for(k = 0; k < allYears[j]->getInfoSize() ; k++){
          ...
          //also considers the string passed into the paramter from the user representing a region
